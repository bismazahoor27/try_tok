#!/usr/bin/env python3
"""
FragSMILES RNN — 1-Fold, selectable BPE variant
Run with:
    python one_fold_moses_bpe_diff.py --bpe-variant bpe_right_linker
    python one_fold_moses_bpe_diff.py --bpe-variant bpe

The --bpe-variant flag controls which BPE module is used:
  bpe               — original (linker runs ARE merged)
  bpe_right_linker  — strict (linker runs are NOT merged)

All outputs are namespaced by variant under ~/working/<variant>/
so both runs can coexist without overwriting each other.
"""

# ─────────────────────────────────────────────────────────────────────────────
# CLI — parse first, before any heavy imports
# ─────────────────────────────────────────────────────────────────────────────
import argparse

_parser = argparse.ArgumentParser(
    description="FragSMILES RNN 1-fold benchmark with selectable BPE variant."
)
_parser.add_argument(
    '--bpe-variant',
    choices=['bpe', 'bpe_right_linker'],
    default='bpe_right_linker',
    help="BPE module to use (default: bpe_right_linker).",
)
_args = _parser.parse_args()
BPE_VARIANT = _args.bpe_variant          # e.g. "bpe_right_linker"
BPE_MODULE  = f"chemicalgof.{BPE_VARIANT}"  # e.g. "chemicalgof.bpe_right_linker"

print(f"BPE variant : {BPE_VARIANT}")
print(f"BPE module  : {BPE_MODULE}")

# ─────────────────────────────────────────────────────────────────────────────
# IMPORTS
# ─────────────────────────────────────────────────────────────────────────────
import subprocess, sys, shutil, os, types, gc, time, json, glob, importlib
...
# Dynamic BPE import — controlled by --bpe-variant
_bpe_mod   = importlib.import_module(BPE_MODULE)
BPETrainer  = _bpe_mod.BPETrainer
BPETokenizer = _bpe_mod.BPETokenizer
print(f"BPETrainer / BPETokenizer loaded from {BPE_MODULE} ✓")
...
# ─────────────────────────────────────────────────────────────────────────────
# PHASE 2: Train BPE on the full corpus using the selected variant
# ─────────────────────────────────────────────────────────────────────────────
if os.path.exists(BPE_MERGES_PATH) and os.path.exists(VOCAB_PATH) and os.path.exists(TOKENS_OUT):
    print(f"\nLoading existing BPE vocab ({BPE_VARIANT}) ...")
    ...
    print(f"\nTraining BPE [{BPE_VARIANT}] on full corpus "
          f"(max_merges={BPE_MAX_MERGES}, min_freq={BPE_MIN_FREQ}) ...")
    t_bpe   = time.time()
    trainer = BPETrainer()
...
# bpe_module is passed as a string so the loky worker can re-import it cleanly.
# ─────────────────────────────────────────────────────────────────────────────
def decode_sequences(all_seqs, bpe_merges_path, bpe_module, n_jobs):
    def decode_worker(token_list):
        try:
            ...
            try:
                ...
                _mod = importlib.import_module(bpe_module)
                _BPE_TOK = _mod.BPETokenizer.load(bpe_merges_path)
...
print(f"\n{'='*60}")
print(f"  1 Fold  [BPE variant: {BPE_VARIANT}]")
...
for fold, (train_idx, val_idx) in enumerate(kf, start=1):
    ...
    for epoch in range(N_EPOCHS):
        ...
        fold_history.append({'fold': fold, 'epoch': epoch+1, 'train_loss': train_loss,
                              'bpe_variant': BPE_VARIANT})
        ...
        if train_loss < best_loss:
            if best_loss < float('inf') and os.path.exists(save_best):
                ...
                print(f"  Promoted previous best (loss={best_loss:.4f}) to second best ✓", flush=True)
            ...
            print(f"  Saved new best (loss={best_loss:.4f}) at epoch {epoch+1} ✓", flush=True)
            ...
            print(f"  Saved new second best (loss={second_best_loss:.4f}) at epoch {epoch+1} ✓", flush=True)
    ...
    # ── Decode — pass BPE_MODULE so workers import the right class ────────────
    print(f"  Decoding sequences → SMILES  [{BPE_VARIANT}] ...")
    valid_smiles  = decode_sequences(all_seqs, BPE_MERGES_PATH, BPE_MODULE, N_JOBS)
    ...
    metrics['fold']            = fold
    metrics['bpe_variant']     = BPE_VARIANT
    ...
    print(f"\n  Fold {fold} MOSES metrics  [{BPE_VARIANT}]:")
    for k, v in metrics.items():
        if k not in ('fold', 'bpe_variant'):
...
# ─────────────────────────────────────────────────────────────────────────────
print(f"\n{'='*60}")
print(f" RESULTS  ({N_FOLDS} fold, BPE variant: {BPE_VARIANT})")
print(f"{'='*60}")
