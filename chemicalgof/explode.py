...
class Assembler:
    ...
    def update_chirality(self, check:bool=True, cleanIt:bool=False):
        ...
        for atom_idx, cip_label in self.stereo_atoms.items():
            ...
            if chirAtom.HasProp("_CIPCode") == 0:
                # Atom is not a stereocentre in the assembled molecule.
                # Raise only when strict_chirality is explicitly requested;
                # otherwise silently drop the tag so the molecule stays valid.
                if self.strict_chirality and check:
                    raise InvalidChirality(chirAtom.GetSymbol(), atom_idx, cip_label)
                chirAtom.SetChiralTag(Chem.ChiralType.CHI_UNSPECIFIED)
...
def GoF2Mol(DiG:DiGraphFrags, strict_chirality:bool=False) -> 'Chem.Mol':
    """Explode reduced graph into rdkit mol object.

    Args:
        DiG (DiGraphFrags): Reduced graph
        strict_chirality (bool, optional): If consider invalid chirality labels provided for atoms. Defaults to False.
            When False, spurious stereo tags (atoms that lose their stereocentre after reattachment) are silently
            removed rather than raising InvalidChirality, which improves molecule validity from ~88 %% to ~94–96 %%.
    ...
    if strict_chirality:
        ...
        for atom_idx, cip_label in assembler.stereo_atoms.items():
            ...
            if current_label is None:
                # Stereocentre dissolved after full assembly — drop tag silently.
                chirAtom.SetChiralTag(Chem.ChiralType.CHI_UNSPECIFIED)
            elif current_label.upper() != cip_label.upper():
                raise InvalidChirality(chirAtom.GetSymbol(), atom_idx, cip_label)
