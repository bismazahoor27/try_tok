...
def classify_fragment_type(smiles: str) -> str:
    """Classify a fragment SMILES as 'ring', 'linker', or 'sidechain'.

    A fragment is 'ring' if it contains any ring atom.  A fragment with no
    ring atoms and only one potential connection point is classified as
    'sidechain'.  Everything else (no ring, multiple connection points) is a
    'linker'.
    """
    ...
    if m.GetRingInfo().NumRings() > 0:
        return 'ring'
    pot_linkers = [a for a in m.GetAtoms() if a.GetTotalNumHs() > 0]
    if len(pot_linkers) <= 1:
        return 'sidechain'
    return 'linker'
...
class FragNode:
    ...
    def __init__(self, smiles:str|None = None, chirality: dict[int,str] | None = None, parent : Union['DiGraphFrags', None] = None, node_type: str = 'unknown'):
        # NOTE parent is set when node is added to Graph at the first time.
        self.parent = parent
        # node_type: 'ring', 'linker', 'linker_run', 'sidechain', 'unknown'
