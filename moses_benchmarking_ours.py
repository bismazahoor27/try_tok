#!/usr/bin/env python3
"""
FragSMILES RNN — 5-Fold Cross Validation
...
N_EPOCHS       = 30
N_FOLDS        = 1
N_SAMPLES      = 15000      # sequences sampled per fold
CHUNK          = 200_000
BPE_MAX_MERGES = 1000
BPE_MIN_FREQ   = 100
...
for fold, (train_idx, val_idx) in enumerate(kf.split(all_indices), start=1):
    ...
    for epoch in range(N_EPOCHS):
        ...
        print(f"  Fold {fold} | Epoch {epoch+1:02d}/{N_EPOCHS} | "
              f"Train: {train_loss:.4f} | Val: {val_loss:.4f} | Time: {elapsed:.1f}s", flush=True)
        print(f"  {'='*50}\n", flush=True)
        ...
        if val_loss < best_loss:
            ...
            torch.save({'model_state': {k: v.cpu() for k, v in m.state_dict().items()}, 'vocab': vocab,
                        'token2idx': token2idx, 'val_loss': val_loss,
    ...
    print(f"\n  Fold {fold} training complete. Best val loss: {best_loss:.4f} ✓")
